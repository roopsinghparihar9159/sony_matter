...
class transcoder(object):
	...
	def main(self):
		...
		if(codec_name == 'h264'):
			input_opts=" -hwaccel_device %s -hwaccel cuvid -c:v h264_cuvid " % ('2')
			cudaDownloadOpts="hwdownload,"
		elif(codec_name == 'mpeg2video'):
			input_opts=" -hwaccel_device %s -hwaccel cuvid -c:v mpeg2_cuvid " % ('2')
			cudaDownloadOpts="hwdownload,"
		mp4files=''
		for data in jsonData[0]['content_id']:
			...
			transcode_opts+=f" -map \"0:v\" -c:v h264_nvenc -r:v {fps} -g:v {gop} -b:v {vbitrate} -maxrate {mrate} -preset:v fast -rc:v vbr -rc-lookahead:v 32 {scale_opts} -no-scenecut 1 -metadata service_provider=multitv -metadata service_name=multitv -map 0:a -c:a aac -b:a {ab} -ar 48000 {op_fn} "
			mp4files+=' '+op_fn
		print(mp4files)
