...
from dbconn import *
from conf import *
import os, boto3, json, m3u8, urllib.parse, DownloadProgressBar, logging, iso639, re, ast, glob, string, random, uuid, xmltodict, base64
...
class transcode(object):
	...
	def getFile(self,srturl,srt_fileLANG):
		...
		video_file_name = video_file_name.replace(".srt",f"_{srt_fileLANG}.srt") if ".srt" in video_file_name else (video_file_name.replace(".SRT", f"_{srt_fileLANG}.srt") if ".SRT" in video_file_name else "")
	...
	# def postEncodeStatus(self,content_id,duration,cfurl,baseUrl,gifurl,thumbURL):
	# 	url=f"https://{baseUrl}/crons/tanscoded_url"
	# 	body = {'content_id': content_id,'duration':duration,'abr':cfurl,'gif':gifurl,'thumb':thumbURL}
	# 	r=post(url,json=body)
	# 	print(r.text)
	# 	return(True)
	...
	def getDrmKey(self, content_id):
		...
		kid=uuid.uuid4()
		url = f"https://cpix.ezdrm.com/keygenerator/cpix.aspx?k={kid}&u={EZusername}&p={EZpassword}&c={content_id}"
		r=get(url)
		response = r.text
		print(r.text)
	...
	def transcoder(self):
		...
		if c_type == "audio":
			...
			for flavor in data:
				...
				cmd = f"ffmpeg -y -i '{file_url}' -c:a aac -ab {ab} {outDr}/{outFilepath}_{ab}.m4a"
				p=Popen(cmd,stdout=PIPE,stderr=PIPE,shell=True)
				out,err=p.communicate()
				...
				pack_cmd_opts += f" 'in={outDr}/{outFilepath}_{ab}.m4a,stream=audio,init_segment={ab}/init.mp4,segment_template={ab}_$Number$.m4s,playlist_name={ab}.m3u8' "
			...
			if(p.returncode != 0):
				status = "P-Failed"
				self.postTranscodingStatus(content_id,status,baseUrl,err.decode('utf-8'))
			...
			if status == "AUD":
				...
				for filename in glob.iglob(uploadDir+"/**", recursive=True):
					...
					if local_source != None:
						...
						print( ok, " -- ",response)
				print( ok, " -- ",response)
				final_url = f"https://art-of-living-nsu.akamaihd.net/{AOL_bucket}/mp3/{outFilepath}/master.m3u8"
				self.DB_updater(content_id,app_id,fn,"transcoder",status,"75%")
				self.DB_updater(content_id,app_id,fn,"uploader",status,"100%")
				self.postEncodeStatusBothnonandnonDRM(content_id,duration,final_url,final_url,base_url,final_url,final_url)
		...
		for flavor in data:
			...
			transcode_opts+=" -map \"0:v\" -c:v libx264 -r:v %s -g:v %s -b:v %s -maxrate %s  -preset:v fast -rc:v vbr  %s -metadata service_provider=multitv -metadata service_name=multitv -map 0:a -c:a aac -b:a %s -ar 48000 %s " % (fps,gop,vbitrate,mrate,scale_opts,abitrate,op_fn)
			count+=1
