...
def hits_by_os():
    ...
    path = f"/reporting-api/v1/reports/hits-by-os/versions/1/report-data?start={start_date}&end={end_date}&objectIds={ObjectIds}&metrics=successfulHits%2CsuccessfulHitsPercent&dataWrapNumberOfItems=2&dataWrapLabel=Other"
...
def hits_by_time():
    ...
    if objectIds is not None:
        path = f"/reporting-api/v1/reports/hits-by-time/versions/1/report-data?start={start_date}&end={end_date}&objectIds={objectIds}&interval=FIVE_MINUTES&metrics=edgeHitsPerSecond,edgeHitsPerSecondMax&filters=delivery_type=secure,ip_version=ipv4,ip_version=ipv6"
        if(request.method == 'GET'):
            baseurl = base_url
            s = requests.Session()
            s.auth = EdgeGridAuth(
...
def hits_by_cpcode():
    ...
    if objectIds is not None:
        path=f"/reporting-api/v1/reports/hits-by-cpcode/versions/1/report-data?start={start_date}&end={end_date}&objectIds={objectIds}&metrics=edgeHits,hitsOffload&filters=delivery_type=secure,ip_version=ipv4,ip_version=ipv6"
        if(request.method == 'GET'):
            baseurl = base_url
            s = requests.Session()
            s.auth = EdgeGridAuth(
...
def bytes_by_httpversion():
    ...
    if objectIds is not None:
        path = f"/reporting-api/v1/reports/bytes-by-httpversion/versions/1/report-data?start={start_date}&end={end_date}&objectIds={objectIds}&metrics=edgeBytes"
...
def bytes_by_cpcode():
    ...
    path = f"/reporting-api/v1/reports/bytes-by-cpcode/versions/1/report-data?start={start_date}&end={end_date}&objectIds={objectIds}&metrics=bytesOffload,edgeBytes"
...
def hits_by_browser():
    ...
    startdate = current_date - timedelta(days=day_no)
    start_date = startdate.strftime('%Y-%m-%dT%H:%MZ')
    # url=http://127.0.0.1:5000/hits_by_browser?days=19&objectIds=1349555
    if objectIds is not None:
        path = f"/reporting-api/v1/reports/hits-by-browser/versions/1/report-data?start={start_date}&end={end_date}&objectIds={objectIds}&metrics=successfulHits,successfulHitsPercent&dataWrapNumberOfItems=2&dataWrapLabel=Other"
        if(request.method == 'GET'):
            baseurl = base_url
            s = requests.Session()
            s.auth = EdgeGridAuth(
...
def hits_by_tophostnameandtime():
    ...
    if objectIds is not None:
        path = f"/reporting-api/v1/reports/hits-by-tophostnameandtime/versions/1/report-data?start={start_date}&end={end_date}&interval=FIVE_MINUTES&objectIds={objectIds}&metrics=maxEdgeHits&filters=ca%3Dcacheable%2Cca%3Dnon_cacheable%2Cdelivery_type%3Dsecure"
        if(request.method == 'GET'):
            baseurl = base_url
            s = requests.Session()
            s.auth = EdgeGridAuth(
