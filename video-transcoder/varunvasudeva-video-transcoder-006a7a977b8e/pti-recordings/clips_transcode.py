...
class Read_Xfer(object):
	...
	def transcoder(self,fn):
		...
		for flavor in self.transcode_profile:
			...
			transcode_opts+=" -map \"[out%s]\" -c:v h264_nvenc -r:v %s -g:v %s -b:v %s -maxrate %s -qmax:v 22.0 -preset:v medium -rc:v vbr -rc-lookahead:v 32 -no-scenecut 1 -metadata service_provider=multitv -metadata service_name=multitv -map 0:a -c:a aac -b:a %s -ar 48000 %s " % (count,fps,gop,vbitrate,mrate,abitrate,op_fn)
			count+=1
